...
| | 退出码 | 说明 |
|---|:--:|---|
| `--dry-run` | **0** | 所有 URL 可达，计划算得出来 |
| `bootstrap` | **0** | 实测 59 成功 · 3 跳过 · 0 失败 |
| `verify` | **2** | 容器里必然是 2，见下 |

`verify` 在容器里返回 2（warn）是正常的，两条 warn 都不是 bug：

- **git 身份未填写** —— 刻意的。脚本绝不猜也绝不自动填。
- **`tput: No value for $TERM`** —— `docker run` 不带 `-t` 就没有 TTY，
  `zsh -i` 里有东西调 `tput` 会往 stderr 抱怨一句。真实终端里不会出现。

容器里没有图形会话，所以 70-desktop 整步跳过；字体和 GUI 检查在
`--profile minimal` 下不参与 —— 也都是预期内的。
